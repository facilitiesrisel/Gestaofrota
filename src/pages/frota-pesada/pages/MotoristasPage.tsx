...
const MotoristasPage: React.FC = () => {
  ...
  const handleSave = async () => {
    if (currentMotorista.login && currentMotorista.nome) {
      ...
      // If renaming (Login changed), delete old record first
      if (editingLogin && editingLogin !== currentMotorista.login) {
          await deleteMotorista(editingLogin);
      }

      await saveMotorista(currentMotorista as Motorista);
      await loadData(true);
  ...
  const processedMotoristas = useMemo(() => {
      ...
      const augmentedResult = result.map(m => {
          ...
          const driverNameClean = cleanString(m.nome);
          
          // Match logic: Login matches responsavelCodigo OR Name matches responsavelNome
          const driverMultas = allMultas.filter(multa => {
              const respCodClean = cleanString(multa.responsavelCodigo);
              const respNomeClean = cleanString(multa.responsavelNome);
              return (driverLoginClean && respCodClean && respCodClean === driverLoginClean) ||
                     (driverNameClean && respNomeClean && respNomeClean === driverNameClean) ||
                     (driverNameClean && respCodClean && respCodClean === driverNameClean) ||
                     (driverLoginClean && respNomeClean && respNomeClean === driverLoginClean);
          });
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 relative flex flex-col h-full overflow-hidden">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 shrink-0 bg-white/80 px-3 py-2 rounded-xl border border-slate-200/70 shadow-2xs backdrop-blur-xs">
        <div>
          <h2 className="text-base sm:text-lg font-black bg-clip-text text-transparent bg-gradient-to-r from-blue-800 to-indigo-700 tracking-tight uppercase leading-none">
            Gestão de Motoristas
          </h2>
          <p className="text-slate-500 text-[10px] sm:text-[10.5px] font-semibold mt-0.5 leading-none">
            Controle de condutores e pontuação {filters.year ? `(${filters.year})` : '(Geral)'}
          </p>
        </div>
        ...
        <div className="flex items-center space-x-1.5 w-full sm:w-auto justify-end flex-wrap gap-y-1">
            ...
            <button onClick={() => loadData(true)} className="bg-white hover:bg-gray-50 text-blue-700 border border-blue-600/30 px-2 py-1.5 rounded-lg flex items-center shadow-2xs transition-all active:scale-95 font-bold text-[11px]" title="Recarregar dados">
                <RefreshCw size={13} />
            </button>
            <button onClick={() => { setCurrentMotorista({ status: 'ATIVO' }); setEditingLogin(null); setIsModalOpen(true); }} className="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 rounded-lg flex items-center shadow-2xs transition-all active:scale-95 font-bold text-[11px] uppercase">
                <Plus size={13} className="mr-1" /> Novo Motorista
            </button>
        </div>
      </div>
      ...
      {/* Search */}
      <div className="bg-white px-3 py-1.5 rounded-xl shadow-2xs border border-gray-200/80 flex items-center justify-between gap-2.5 transition-all focus-within:ring-2 focus-within:ring-blue-500/20 shrink-0">
        <div className="flex items-center flex-1">
          <Search className="text-gray-400 mr-2 shrink-0" size={14}/>
          <input 
            type="text" 
            placeholder="BUSCAR POR NOME, LOGIN OU BASE..." 
        ...
        <div className="flex items-center gap-2 text-[10px] text-slate-500 font-semibold border-l border-slate-200 pl-2.5 shrink-0">
          <span>Exibindo: <strong className="text-slate-800">{processedMotoristas.length}</strong> motoristas</span>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden flex flex-col flex-1 h-full min-h-0">
            <div className="overflow-auto flex-1 custom-scrollbar w-full relative">
                <table className="min-w-max border-collapse uppercase w-full text-[10.5px]">
                    <thead className="sticky top-0 z-10 shadow-sm">
                        <tr className="bg-gradient-to-r from-[#022c22] to-risel-green">
                            <th className="px-2 py-2 w-16 text-center text-[9px] font-bold text-white/90 uppercase tracking-wider border-r border-white/10 sticky left-0 z-20 bg-[#022c22]">Ações</th>
                            <th onClick={() => handleSort('status')} className="px-3 py-2 text-left text-[9px] font-bold text-white/90 uppercase tracking-wider cursor-pointer hover:bg-white/10 transition-colors group border-r border-white/10">Status <ArrowUpDown size={10} className="inline ml-1 opacity-50 group-hover:opacity-100"/></th>
                            <th onClick={() => handleSort('login')} className="px-3 py-2 text-left text-[9px] font-bold text-white/90 uppercase tracking-wider cursor-pointer hover:bg-white/10 transition-colors group border-r border-white/10">Login <ArrowUpDown size={10} className="inline ml-1 opacity-50 group-hover:opacity-100"/></th>
                            <th onClick={() => handleSort('nome')} className="px-3 py-2 text-left text-[9px] font-bold text-white/90 uppercase tracking-wider cursor-pointer hover:bg-white/10 transition-colors group border-r border-white/10 w-1/3">Nome <ArrowUpDown size={10} className="inline ml-1 opacity-50 group-hover:opacity-100"/></th>
                            <th onClick={() => handleSort('base')} className="px-3 py-2 text-left text-[9px] font-bold text-white/90 uppercase tracking-wider cursor-pointer hover:bg-white/10 transition-colors group border-r border-white/10">Base <ArrowUpDown size={10} className="inline ml-1 opacity-50 group-hover:opacity-100"/></th>
                            <th onClick={() => handleSort('qtdMultas')} className="px-3 py-2 text-center text-[9px] font-bold text-white/90 uppercase tracking-wider cursor-pointer hover:bg-white/10 transition-colors group border-r border-white/10">Qtd. Multas <ArrowUpDown size={10} className="inline ml-1 opacity-50 group-hover:opacity-100"/></th>
                            <th onClick={() => handleSort('valorMultas')} className="px-3 py-2 text-right text-[9px] font-bold text-white/90 uppercase tracking-wider cursor-pointer hover:bg-white/10 transition-colors group border-r border-white/10">Valor Multas <ArrowUpDown size={10} className="inline ml-1 opacity-50 group-hover:opacity-100"/></th>
                        </tr>
                    </thead>
                    <tbody className="bg-white divide-y divide-gray-100 text-[10px]">
                        {processedMotoristas.map((motorista, idx) => {
                            ...
                            return (
                                <tr key={motorista.login + idx} className={`${rowClass} hover:bg-blue-50/50 transition-colors group`}>
                                    <td className="px-1.5 py-1.5 text-center border-r border-gray-100 sticky left-0 bg-white group-hover:bg-blue-50/50 z-10">
                                        <div className="flex justify-center space-x-1">
                                            <button onClick={() => { setCurrentMotorista(motorista); setEditingLogin(motorista.login); setIsModalOpen(true); }} className="text-gray-400 hover:text-blue-600 transition-colors p-0.5" title="Editar"><Edit size={13}/></button>
                                            <button onClick={() => handleDelete(motorista.login)} className="text-gray-400 hover:text-red-600 transition-colors p-0.5" title="Excluir"><Trash2 size={13}/></button>
                                        </div>
                                    </td>
                                    <td className="px-3 py-1.5 border-r border-gray-100"><span className={`inline-flex items-center px-1.5 py-0.5 rounded text-[8.5px] font-bold uppercase border ${isActive ? 'bg-green-50 text-green-700 border-green-200' : 'bg-red-50 text-red-700 border-red-200'}`}>{motorista.status || 'ATIVO'}</span></td>
                                    <td className="px-3 py-1.5 border-r border-gray-100 text-[9.5px] font-bold font-mono text-gray-700">{motorista.login}</td>
                                    <td className="px-3 py-1.5 border-r border-gray-100 text-[9.5px] font-bold text-gray-700">{motorista.nome}</td>
                                    <td className="px-3 py-1.5 border-r border-gray-100 text-[9px] text-gray-500"><div className="flex items-center"><MapPin size={9} className="mr-1 text-gray-400"/>{motorista.base || '-'}</div></td>
                                    <td className="px-3 py-1.5 border-r border-gray-100 text-[9px] text-center font-bold text-gray-700">{motorista.qtdMultas > 0 ? <span className="bg-red-100 text-red-700 px-1.5 py-0.5 rounded-full">{motorista.qtdMultas}</span> : <span className="text-gray-400">-</span>}</td>
                                    <td className="px-3 py-1.5 border-r border-gray-100 text-[10px] text-right font-bold text-gray-700">{motorista.valorMultas > 0 ? <span className="text-red-600">{formatCurrency(motorista.valorMultas)}</span> : <span className="text-gray-400">-</span>}</td>
                                </tr>
                            )
                        })}
                    </tbody>
