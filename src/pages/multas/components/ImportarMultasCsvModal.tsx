...
export const ImportarMultasCsvModal: React.FC<ImportarMultasCsvModalProps> = ({
  ...
  const handleConfirmImport = async () => {
    ...
    const multasParaGravar: Multa[] = itemsToImport.map((item) => {
      return {
        id: String(item.ait || item.id),
        ait: String(item.ait || '').trim(),
        placa: String(item.placa || '').toUpperCase().trim(),
        ...
        descricaoInfracao: String(item.descricaoInfracao || '').trim(),
        pontosCnh: Number(item.pontosCnh) || 0,
        responsavelCodigo: '',
        responsavelNome: '',
        orgaoAutuador: '',
